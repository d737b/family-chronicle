...
import type { Entry, EntryType, AppConfig, MemberConfig } from '@/lib/types';
...
// ── 類型設定 ─────────────────────────────────────────────────────────────────
const TYPE_OPTIONS: { id: EntryType | 'all'; label: string; icon: string }[] = [
  { id: 'all',    label: '全部', icon: '✦' },
  { id: 'photo',  label: '照片', icon: '📷' },
  { id: 'video',  label: '影片', icon: '🎬' },
  { id: 'text',   label: '文字', icon: '✏️' },
  { id: 'link',   label: '網頁', icon: '🔗' },
  { id: 'place',  label: '地點', icon: '📍' },
  { id: 'social', label: '社群', icon: '💬' },
];
...
export default function AppClient() {
  ...
  const fetchEntries = useCallback(async (cfg: AppConfig) => {
    ...
    try {
      ...
      const json = await res.json() as { ok: boolean; data?: Entry[]; error?: string; errors?: string[] };
      if (json.ok && json.data) {
        setEntries(json.data.sort((a, b) => b.date.localeCompare(a.date)));
      } else {
  ...
  // ── Filter + Bopomofo search ──────────────────────────────────────────────
  const filtered = (() => {
    let pool = entries;
    if (activeMember !== 'all') pool = pool.filter(e => e.memberId === activeMember);
    if (activeType   !== 'all') pool = pool.filter(e => e.type === activeType);
    if (!query.trim()) return pool.map(item => ({ item, score: 1 }));
    return bpmfSearch(pool, query, e => [e.title, e.body, ...e.tags, e.location ?? '', e.platform ?? '']);
  })();
    ...
    <div className="min-h-screen bg-[#F2EDE6] font-sans">
      ...
      <header className="sticky top-0 z-40 bg-[#FFF9F5]/95 backdrop-blur border-b border-[#3D3530]/10">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          {/* Refresh */}
          <button
            onClick={() => config && fetchEntries(config)}
            disabled={loading}
        ...
        <div className="max-w-3xl mx-auto px-4 pt-3 pb-1 flex gap-3 overflow-x-auto scrollbar-hide">
          ...
          >
            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg border-2 transition-all
              ${activeMember === 'all' ? 'border-[#7A6B5E] shadow-sm' : 'border-transparent bg-[#E8EDE7]'}`}>
              👨‍👩‍👧‍👦
            </div>
            <span className={`text-[9px] font-medium transition-colors ${activeMember === 'all' ? 'text-[#3D3530] font-bold' : 'text-[#9B8F88]'}`}>
              所有人
            </span>
          </button>
          {memberList.filter(m => m.notionDbId).map(m => (
            <button key={m.id} onClick={() => setActiveMember(m.id)} className="flex flex-col items-center gap-1 flex-shrink-0">
              ...
              <span className={`text-[9px] font-medium transition-colors ${activeMember === m.id ? 'text-[#3D3530] font-bold' : 'text-[#9B8F88]'}`}>
                {m.name}
              </span>
            </button>
          ))}
      ...
      {/* ── TYPE FILTER ── */}
      <div className="max-w-3xl mx-auto px-4 pt-3 flex gap-2 overflow-x-auto scrollbar-hide">
        {TYPE_OPTIONS.map(t => (
          <button
            key={t.id}
            onClick={() => setActiveType(t.id as EntryType | 'all')}
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[11px] font-medium border transition-all
              ${activeType === t.id
                ? 'bg-[#9BA89A] text-white border-[#9BA89A]'
                : 'bg-white text-[#6B5F58] border-[#3D3530]/12 hover:border-[#9BA89A]/50'}`}
          >
            {t.icon} {t.label}
          </button>
        ))}
      ...
      <main className="max-w-3xl mx-auto px-4 pt-4 pb-20">
        ...
        <div className="flex items-center justify-between mb-3">
          <span className="text-[10px] text-[#9B8F88] tracking-widest uppercase">
            {query ? `搜尋「${query}」` : (activeMember !== 'all' ? memberList.find(m => m.id === activeMember)?.name ?? '全部' : '所有記憶')}
            {' · '}{filtered.length} 筆
          </span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="rounded-xl bg-white border border-[#3D3530]/10 overflow-hidden animate-pulse">
                ...
                <div className="p-3">
                  <div className="h-3 bg-[#E8EDE7] rounded mb-2 w-3/4" />
                  <div className="h-2 bg-[#E8EDE7] rounded w-1/2" />
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedEntry && (
        <EntryDetail
          entry={selectedEntry}
          member={memberList.find(m => m.id === selectedEntry.memberId)}
